...
- **A. 搜菜找货（菜 → 货，引流）**：顾客搜一道菜（如"麻婆豆腐"），给出菜谱 + 所需食材清单，每样标注**是否有货 / 分类**（**不显示价格**——见下"价格展示决策"），引导"我想做这道菜"→"来东方买齐"。
...
3. 每样食材一行：`【食材名 · 用量】` → `【有货 · 分类】` 或 `【暂缺】`。**不显示价格。**
4. 底部汇总：「这道菜共 N 样食材，东方超市有 M 样 · 备齐就能开做」+「♡ 想做」收藏。

> **价格展示决策（2026-06-15，店主定）**：菜谱视图**一律不显示价格**。原因：① 标价让菜谱像价目表，削弱"激发食欲"的定位；② 调料按整瓶计价会把"一道菜"的头部金额抬得很高（如麻婆豆腐含整瓶芝麻油/生抽 → $38 起），与"勾起购买欲、别吓人"的目标相反。改为只呈现**有货/暂缺 + 分类**来引导到店。`products.json` 仍保留 `price`/`price_unit` 字段，`associateRecipe` 仍计算（便于将来需要时恢复或用于店主侧 B），但前端不渲染。称重/市价的特殊处理因此仅保留在逻辑层、不影响展示。
...
- 供应时效：`products.json` 每日导出，页脚标注"以店内实际供应为准"。（菜谱不显示价格，故无隔夜调价纠纷。）
